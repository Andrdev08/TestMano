...
class MainActivity : AppCompatActivity() {
    ...
    fun onClear(view: View) {
        if (dot && binding.numberScreen.lastZero.text.length > 0) {
         clearAfterDot()
        } else {
         clearBeforeDot()
        }
    ...
    private fun setAED(view: View) {
        if (!lastN && !dot && view != binding.btnZero) {
            binding.numberScreen.AED.text = "AED"
        }
    }

    private fun setNumberAfterDot(view: View) {
    ...
    private fun setNumberBeforeDot(view: View) {
    ...
    private fun clearBeforeDot(){
        if (lastN && binding.numberScreen.firstZero.text.length > 0) {
